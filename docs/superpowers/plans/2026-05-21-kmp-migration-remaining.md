...
- 强制性 subagent 生命周期规则：只要启动了 subagent，主 agent 必须等待所有仍存活的 subagent 完成；只有确认任务已经不再需要时，才能主动关闭 subagent，并在回复或相关审查文档中记录关闭原因。不能留下孤儿 subagent。任一 subagent 仍存活时，不能提交、不能宣布切片完成、不能把本地判断当作最终结论，也不能绕过它继续推进会受该结论影响的迁移。
