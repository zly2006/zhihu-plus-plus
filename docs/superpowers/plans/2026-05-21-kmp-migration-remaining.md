...
- 强制性 subagent 生命周期规则：只要启动了 subagent，主 agent 必须等待所有仍存活的 subagent 完成；等待完成是默认且必须执行的路径，不能因为本地已有判断、等待较久、或想继续推进而绕过。不能留下孤儿 subagent。任一 subagent 仍存活时，不能提交、不能宣布切片完成、不能把本地判断当作最终结论，也不能绕过它继续推进会受该结论影响的迁移。每次准备提交、宣布完成、停止当前回合或发送最终回复前，都必须盘点本回合启动过的 subagent：仍存活的必须 `wait_agent` 到完成，不能静默遗留。只有用户明确取消、任务已作废、或该 subagent 的结论已不再可能影响当前工作时，才能主动关闭 subagent，并必须在回复或相关审查文档中记录关闭原因。
