...
在 412 × 892 的 Compose Desktop JVM 画布上，最终 16 类稳定场景的中位数全部低于 30 ms，最大值为 25.6 ms；全部原始样本低于 100 ms，至少 70% 低于 50 ms。由项目真实公式语料中最长 80 条公式组成的同一篇压力文章，首次向前滚动中位数从 341.7 ms 降到 27.7 ms。

## 优化前后

下表使用相同的 selectable + deferred 生产路径、相同的最长 80 公式文章和相同的 700 px 分步滚动边界。每一步都会等待 Compose 完成重组、布局和场景绘制，再截图确认像素；Skia 把整张位图回读到 CPU 的测试专有开销不计入滚动延迟。
...
| 返回滚动 P90 | 108.7 ms | 25.7 ms | -76.4% |
...
| --- | ---: | ---: | ---: | ---: |
| 向前，包含公式准备、布局和 draw 验证 | 27.7 ms | 41.2 ms | 47.5 ms | 40 / 40 |
| 返回，命中文档缓存 | 22.6 ms | 25.7 ms | 32.3 ms | 40 / 40 |
| 公式准备期间的独立 UI 响应 | 19.3 ms | 30.0 ms | 46.4 ms | 40 / 40 |

普通正文、格式文本、链接、长文、列表、代码、行内与块公式、表格及混合文档共 16 类场景中：

- 16 / 16 场景中位数低于 30 ms，最大中位数为 25.6 ms；
- 所有原始样本低于 100 ms；
- 至少 70% 原始样本低于 50 ms；
...
4. selectable/static 正文使用 `SubcomposeLayout`：普通内容只预取半个视口，块公式文章保留经过压力验证的 1.5 个视口；列表项和表格行也使用同一套递归 deferred，而不是在一个可见顶层块中全量组合数百行。
...
功能回归还覆盖真实公式像素、行内公式尺寸更新、缓存跨组合回收、纯链接与公式混合段落链接点击、300 项递归列表和 200 行表格的尾部物化，以及外层滚动容器中的脚注定义跳转与返回。Android 定向回归另覆盖 200 段混合文档跨卸载全选顺序、重新挂载后的高亮/复制，以及 Activity stop/recreate 时 Bundle key 的 Parcelable 安全性。
