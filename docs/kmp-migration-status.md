...
- Do not leave spawned subagents alive while continuing to final decisions. If a subagent is started, wait for every live subagent to complete. Only close one when its task is explicitly no longer needed, and record why. Before committing, declaring completion, stopping the turn, or sending a final response, explicitly account for all subagents started in the turn: every live one must be completed via `wait_agent`, or closed via `close_agent` with a recorded reason. Before that, do not commit, declare a slice complete, treat the local decision as final, or continue work that depends on the pending conclusion.
