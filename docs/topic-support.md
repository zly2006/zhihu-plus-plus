...
话题首屏按任务频率分配空间：关注是主操作，发想法是次操作；两者始终在同一行，宽度为 3:2，不能让低频创作按钮独占整行。长简介沿用问题详情的完整折叠视觉契约：三行裁剪、末端渐变、展开按钮覆盖在渐变区、展开/收起时恢复空间；短简介不显示按钮，也不预留折叠区。真实 APK 已逐项核对折叠、展开、再次收起和按钮组坐标，展开过程不改变操作层级或造成换行。
