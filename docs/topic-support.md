...
官方 Android 11.4.0（API 31 AVD）的顶层话题页可见“讨论 / 想法 / 待回答”、关注入口和“发想法”操作。Reqable 抓包及登录请求验证了想法最热、最新分别使用 `api.zhihu.com/v5.1/topics/{id}/feeds/pin-hot` 和 `pin-new`；客户端按已观察的窄模型解析字符串 ID、想法文本、作者、赞同/评论与导航，不把字符串正文误套进既有的想法内容数组模型。三个列表都沿用项目连续列表的触底自动分页，不增加显式“加载更多”操作。浏览/讨论量没有进入已验证详情字段，因此不伪造展示。
