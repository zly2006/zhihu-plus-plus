...
| `answerSwitchMode` | 回答切换手势 | `off` / `vertical` / `horizontal` | vertical/horizontal 均使用进程内 Pager 切换，不走路由栈翻页 |
| `answerSwitchSensitivity` | 回答切换灵敏度 | 上下/左右 pager 翻页触发阈值 | 默认 1.0，数值越高触发距离越短；关闭手势时设置项隐藏 |
