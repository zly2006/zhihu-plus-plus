...
| `collectionDirectBrowse` | 收藏直达浏览（测试） | 底栏收藏夹进入内容瀑布流，支持切换、删除、刷新、搜索已加载内容及随机排序 | 默认关闭，关闭时保留原收藏夹列表入口 |

### 收藏直达浏览测试功能边界

- 页面从非活动状态重新进入活动状态时，会同时刷新收藏夹列表和当前收藏夹内容；用户也可以在内容区下拉刷新。
- 搜索只过滤客户端当前已经分页加载的标题、摘要、详情和作者，不新增未经验证的服务端搜索接口；后续分页加载的内容会自动加入搜索范围。
- 随机模式使用收藏夹与本轮随机种子生成稳定排序，Compose 重组不会让卡片反复跳位；切回顺序模式会恢复接口返回顺序。
