...
import androidx.compose.foundation.text.selection.getTextDirectionForOffset
...
import com.hrm.markdown.renderer.selection.selectionLayoutShouldRecompute
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.launch

/** A bridge class between user interaction to the text composables for text selection. */
internal class MarkdownSelectionManager(private val selectionRegistrar: MarkdownSelectionRegistrarImpl) {
    ...
    /**
     * When set to true, it will make MarkdownSelectionManager ignore a copy key event (Ctrl/Cmd + C). It's
    ...
    init {
        ...
        selectionRegistrar.onSelectionUpdateStartCallback =
            { isInTouchMode, layoutCoordinates, rawPosition, selectionMode ->
                ...
                val position =
                    if (textRect.markdownContainsInclusive(rawPosition)) {
                        rawPosition
                    } else {
                        ...
                        val newSelection =
                            Selection(
                                start =
                                    AnchorInfo(
                                        direction =
                                            textLayout.getTextDirectionForOffset(newSelectionRange.start),
                                    ...
                                    AnchorInfo(
                                        direction =
                                            textLayout.getTextDirectionForOffset(newSelectionRange.end),
     ...
     */
    internal fun getAnchorSelectable(anchor: AnchorInfo): MarkdownSelectable? {
    ...
    private fun updateHandleOffsets() {
        ...
        val markdownVisibleBounds = containerCoordinates.markdownVisibleBounds()
        this.startHandlePosition =
            startLayoutCoordinates?.let { handleCoordinates ->
                ...
                position.takeIf {
                    draggingHandle == Handle.SelectionStart || markdownVisibleBounds.markdownContainsInclusive(it)
                }
            }
        ...
        this.endHandlePosition =
            endLayoutCoordinates?.let { handleCoordinates ->
                ...
                val position =
                    containerCoordinates.localPositionOf(handleCoordinates, handlePosition)
                position.takeIf {
                    draggingHandle == Handle.SelectionEnd || markdownVisibleBounds.markdownContainsInclusive(it)
                }
            }
    }
    ...
    internal fun selectAllInSelectable(
        ...
        val newSelection =
            selectionRegistrar.sort().fastFold(null) {
                ...
                mergeMarkdownSelection(mergedSelection, selection)
            }
    ...
    internal fun isEntireContainerSelected(): Boolean {
        val selectables = selectionRegistrar.sort()
        ...
        val newSubSelections =
            mutableLongObjectMapOf<Selection>().apply {
                selectables.fastForEach { selectable ->
                    // Keep the nullable type explicit. With this internal Foundation API, an
                    // Elvis expression compiles to error.NonExistentClass in Android DEX.
                    val subSelection: Selection? = selectable.getSelectAllSelection()
                    if (subSelection == null) return@fastForEach
    ...
    internal fun isNonEmptySelection(): Boolean {
        ...
        return selectionRegistrar.sort().fastAny { selectable ->
    ...
    internal inline fun forEachSelectableWithSelection(
        ...
        val sortedSelectables = selectionRegistrar.sort()
    ...
    private fun getContentRect(): Rect? {
        ...
        val selectedRegionRect =
            getMarkdownSelectedRegionRect(selectableSubSelections, containerCoordinates)

        if (selectedRegionRect == invertedInfiniteRect) return null

        val visibleRect = containerCoordinates.markdownVisibleBounds().intersect(selectedRegionRect)
    ...
    internal fun updateSelection(
        ...
        // The explicit null check is intentional for the same Kotlin/D8 internal-type boundary as
        // selectAll above; `?: return false` produces a runtime-only NonExistentClass cast.
        val selectionLayout: SelectionLayout? =
            getSelectionLayout(position, previousHandlePosition, isStartHandle)
        if (selectionLayout == null) return false
        if (!selectionLayoutShouldRecompute(selectionLayout, previousSelectionLayout)) {
            return false
        }
...
internal fun mergeMarkdownSelection(lhs: Selection?, rhs: Selection?): Selection? {
...
internal fun calculateMarkdownSelectionMagnifierCenterAndroid(
...
internal fun LayoutCoordinates.markdownVisibleBounds(): Rect {
