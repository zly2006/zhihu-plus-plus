...
### Runtime字段只转发共享能力时，必须整个删除Runtime
根本错误：发现runtime字段只是转发共享能力（`rememberBlocklistManager()`、`rememberSettingsStore()`、`rememberImagePreviewOpener()`、`rememberZhihuWebUrlOpener()`、`rememberPlainTextClipboard()`、`rememberUserMessageSink()`等）时，只是把expect/actual合并成common实现，而不是直接删掉整个runtime让屏幕调用共享能力。

正确做法：逐个检查runtime的每个字段——如果所有字段都只是把共享能力换个名字再转发，直接删掉runtime数据类和remember函数，让屏幕直接调用共享能力。例如`BlockUserConfirmRuntime`只有一个`blockUser`字段，本质就是`rememberBlocklistManager().addBlockedUser()`，应该直接删掉runtime，在`BlockUserConfirmDialog`里直接调用`rememberBlocklistManager()`。

反面例子：把`BlockUserConfirmRuntime`从expect/actual改成common实现——runtime本身就不该存在。
正确例子：直接删掉`BlockUserConfirmRuntime`，在`BlockUserConfirmDialog`里内联`rememberBlocklistManager().addBlockedUser()`。

这个原则的判断标准：把runtime每个字段替换成对应的共享能力调用后，如果屏幕代码变得更简单或一样简单，就该删掉runtime。
