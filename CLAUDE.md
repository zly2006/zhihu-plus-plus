...
用户指定 UI 库或其中的成品组件时，先核对发布包与官方源码的组件入口，直接复用组件及完整交互契约，不能只取模糊、圆角等底层效果再自己拼一个外观近似品。组件仅以官方示例源码提供时，应保留版本、来源、许可证和原实现，业务层只做接入；静态截图不能证明按住、拖动、形变和松手反馈已经实现，必须实际验证连续手势。例子：液态导航应直接使用上游的液滴指示器和手势组件，不能用一个带模糊的行布局加缩放动画代替。
...
布局数量回归测试必须覆盖真正越界边界；如果问题由多个认证/称号触发，夹具至少要包含四项认证，不能用两项样本代替。
状态截图回归必须证明状态确实不同；展开、中间和收起截图不能只按文件名区分，测试应校验布局边界或截图摘要发生变化后再上传，避免把同一帧重复当作三种状态。
...
### 质量过滤配置建模
质量过滤是由赞数、粉丝数、回答数等多个条件组成的规则集合；新增可配置项时必须以统一结构体承载完整规则参数，并让每条既有规则都有对应配置入口。不能只为用户点名的单一指标增加孤立 threshold，导致同一规则中的其他固定条件继续不可配置或语义不一致。
创建 issue 分支前必须先 fetch 并确认最新 `origin/master`，用 `git log origin/master..HEAD` 审计祖先提交；不得从其他 feature 分支直接切新分支。若发现错误基线，必须以远程主线为基准重建并只拣选本次变更。

### 独立功能的工作树隔离

开始独立功能实现前，必须先检查当前分支、工作树和并行修改，并从已核对的主线创建专用 worktree；不能因为当前目录可写或暂时干净，就在其他任务的分支直接实现。发现误写后，只迁移本任务的精确改动，并保留原工作树中其他任务的修改；重叠文件要按具体改动拆分，禁止整文件还原。例子：外观功能与阅读设置同时修改设置页时，应只迁移外观入口和主题代码，保留阅读开关及其测试。

Liquid Glass 的全应用视觉规范、实现边界与验收矩阵以 `docs/liquid-glass-design.md` 为准；局部材质替换不能作为整个主题完成的证据。
