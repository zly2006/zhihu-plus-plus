...
- 只要启动了 subagent，就必须等待仍存活的 subagent 完成，或在明确不再需要时主动关闭并记录原因；不得让 subagent 成为孤儿进程。subagent 未完成前，主 agent 不能自行定稿、提交、宣布该切片完成，或把等待中的审查结果当作可忽略项。若 subagent 超时但仍存活，继续等待、调整任务、或显式关闭；不能绕过它继续做最终决策。
