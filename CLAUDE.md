...
- 对两个以上互不重叠的迁移 lane，默认优先并行推进：能用 subagent 时按文件所有权拆分给 subagent；不能用 subagent 时也要按 lane 批量审查、批量验证，避免串行地反复读同一批文件。并行 subagent 只是缩短等待时间，不是后台托管机制；一旦启动或接手，就必须按上面的生命周期规则等待全部完成并消费结论，不能留下孤儿 agent。
