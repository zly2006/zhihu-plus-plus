...
- 如果某个页面 runtime 只剩打开网页、预览图片、复制文本、显示消息、读写设置、打开系统动作等通用平台能力，必须把这些动作拆成或复用 shared/platform 级 expect/helper，然后删除整个页面 runtime；不能只删一个字段后继续留下空壳。正确例子是页面直接调用通用 `open url` / `open image` 能力；保存图片、分享、字体加载、TTS、账号登录、更新安装、链接卡片预览等才是页面或功能 runtime 的真实保留理由。每次新增这种通用能力后必须 grep 其他 runtime 的同类包装并清理低风险重复点，暂不清理的必须写明语义差异或切片风险。
