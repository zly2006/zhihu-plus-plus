...
- 强制性 subagent 生命周期规则：只要启动了 subagent，主 agent 就必须等待所有仍存活的 subagent 完成；只有确认任务已经不再需要时，才能主动关闭 subagent，并在回复或相关审查文档中记录关闭原因。不得让 subagent 成为孤儿 agent。任一 subagent 仍存活时，主 agent 不能自行做最终决策、不能提交、不能宣布切片完成、不能把等待中的审查结果当作可忽略项，也不能绕过它继续推进会受该结论影响的迁移。若 subagent 超时但仍存活，继续等待、调整任务、或显式关闭并记录原因。
