...
- 迁移中要优先合并重复 runtime 和 environment 能力，尤其是 `ArticleViewModelRuntime` / `PaginationEnvironment` 这类已经高度重叠的平台能力集合；能直接使用已有的 `UserMessageSink`、`SettingsStore`、系统动作 helper、账号/签名 helper、数据库 provider 等通用能力时，必须直接复用，不得为了“页面 runtime”或“平台 runtime”再平白无故包一层转发壳。只有真实语义边界不同、生命周期不同、或参数形态无法共享时，才保留独立 runtime，并在切片记录原因。
