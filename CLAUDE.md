...
业务功能需要访问知乎 Web API 时，必须优先使用项目已有的签名请求封装，例如环境层的 `fetchJson`、`postSigned`、`deleteSigned` 或平台侧已经约定的 fetch 方法，不能在功能代码里直接拼 `HttpClient` 再手动调用底层签名函数。底层 `signZhihuFetchRequest` 只适合作为封装实现细节，或在没有上层封装时经过明确判断后使用。例子：一个作者资料查询 provider 应该接收“已签名 fetch 成员资料”的函数，而不是自己持有客户端和签名回调；否则合并时容易绕过统一 cookies、guest access、错误处理和未来签名策略。
