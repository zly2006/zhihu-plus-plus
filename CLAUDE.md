...
- shared UI 内需要发起页面跳转时，优先直接使用 shared 的 `LocalNavigator.current` 和既有 `NavDestination`，不得为了规避依赖或把导航“留给平台”而额外传入一次性导航 lambda。只有跳转真正依赖 Android `Context`/`Intent`/Activity result/WebView 等平台副作用时，才拆成平台 adapter。
...
- 迁移中要优先合并重复 runtime 和 environment 能力，尤其是 `ArticleViewModelRuntime` / `PaginationEnvironment` 这类已经高度重叠的平台能力集合；能直接使用已有的 `UserMessageSink`、`SettingsStore`、系统动作 helper、账号/签名 helper、数据库 provider 等通用能力时，必须直接复用，不得为了“页面 runtime”或“平台 runtime”再平白无故包一层转发壳。清理 `MyModalBottomSheet.kt`、`CommentScreenComponent.kt` 或其他 runtime/adapter 时，同样先检查现有 shared/platform 能力是否已经覆盖，能删转发就删转发，能合并到既有 environment 就合并，不要新增只改名不增语义的 wrapper。只有真实语义边界不同、生命周期不同、或参数形态无法共享时，才保留独立 runtime，并在切片记录原因。
- 如果某个页面 runtime 只剩打开网页、预览图片、复制文本、显示消息、读写设置、打开系统动作等通用平台能力，必须把这些动作拆成或复用 shared/platform 级 expect/helper，然后删除整个页面 runtime；不能只删一个字段后继续留下空壳。正确例子是某个页面 runtime 只包装 `openWebUrl` 和 `openImage`：这两个动作本质是“打开应用内/知乎网页”和“预览图片”的通用能力，应拆成 shared/platform opener，页面直接调用 opener，整份页面 runtime 壳随即删除，并继续 grep 其他 runtime 里相同的打开网页/图片包装一起清理。保存图片、分享、字体加载、TTS、账号登录、更新安装、链接卡片预览等才是页面或功能 runtime 的真实保留理由。每次新增这种通用能力后必须 grep 其他 runtime 的同类包装并清理低风险重复点，暂不清理的必须写明语义差异或切片风险。
- 迁移任何 runtime 前必须做一次“通用能力反向盘点”：逐个字段判断它是在表达页面独有语义，还是只是把网页打开、图片预览、复制、消息提示、设置读写、系统打开等通用能力改名后再转发。发现后者时，优先把能力提升或复用到 shared/platform 层，再回头批量删除所有只剩转发的 runtime 壳和同类重复包装；不能只按当前页面的编译错误局部补 adapter。根本教训是：runtime 的存在本身不是架构边界，只有真实业务语义、生命周期和平台副作用才是边界。
...
- 迁移后的代码必须和 `master` 分支里 `git mv` 之前的原函数保持强一致：主体结构和关键函数名称必须 100% 相似，关键函数相似度必须达到 75% 以上（只变化空格的行不计入差异）。开始迁移、抽取或提交前必须对照 `master` 原文件检查；不满足该要求的迁移切片全部返工重做，不能继续在错误抽象上叠加修改。
- 每个迁移切片提交前必须留下可复查的 `master` 对照结论：列出对照的原始文件/函数、保留的关键函数名、因平台副作用必须拆出的点，以及相似度或人工结构对照结果。凡是主体结构、关键函数名、调用顺序、导航入口或局部 UI 组织明显偏离 `master`，即使能编译也不得提交；必须先按 `master` 形状返工。
...
### 调试设备登录态和日志前必须先保全现场
当问题涉及真机、AVD、账号 JSON 或 logcat 时，不能先覆盖数据或凭某个设备的异常日志下结论。根本错误是把“能拿到一个现象”误当成“已经有可靠证据”，容易把好的登录态覆盖掉，或把设备/系统层卡顿误判成业务网络请求错误。

正确做法：先备份目标设备原始状态，再校验来源 JSON 的结构和关键字段，只在目标验证设备上清日志、复现、读取日志。举例：需要把一台设备的登录态复制到另一台设备时，应先保存接收方原文件，再验证来源 JSON 可解析且包含必要 cookie，最后在接收方复现并看接收方 logcat。

### 修迁移漏接能力时必须先找既有工具和真实归属
当迁移后某个功能因为平台能力没接回而报错时，不能为了快速止血把保存、渲染、权限、路径等实现整段塞进当前 adapter 或 environment。根本错误是把“这里缺一个调用入口”误解成“这里应该重新实现整套能力”，导致重复代码和职责膨胀。

正确做法：先 grep 既有工具、原始实现和同类调用；已有通用工具就直接委托，缺少的最小通用能力应抽到对应平台工具层，再让 adapter 只做薄转发。例如一个导出流程缺少“保存生成物”能力时，保存到系统相册/下载目录的落地逻辑属于平台文件工具，不属于页面或 environment 的业务流程本体。

修单点 bug 时不得顺手补邻近接口。根本错误是看到同一个 environment 里还有其他默认空实现，就擅自补全无关能力；这会扩大 diff、引入重复实现，并掩盖本次问题真正需要的最小链路。正确做法是只接通当前失败路径已经调用的接口；其他相邻接口即使看起来也不完整，也必须另起任务或先征求确认。

### 修复入口前必须先抽出整条公共链路
当两个平台或两个导出模式都在做“准备数据、加载模板、渲染内容、可选内联资源”这类同构流程时，不能只复用末端的一小段函数，然后让上层继续各自拼一遍流程。根本错误是把“有一个公共渲染函数”误认为“已经复用了导出流程”，实际仍然把数据映射、模板读取、资源内联和失败策略散落在多个入口里。

正确做法：先把整条相同流程抽成一个公共编排函数，平台层只传入真正不同的能力，例如读取资源文本、执行网络请求、落地文件或系统分享。举例：一个页面导出和批量导出都需要生成同一种文档时，公共层应该提供“用资源加载器生成文档”和“生成离线文档”的入口；平台层不应该再次手写“先渲染，再判断是否内联资源”的分支。
