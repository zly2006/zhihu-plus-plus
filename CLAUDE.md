...
- 迁移后的代码必须尽量保持和 `master` 分支里 `git mv` 之前的原函数一致：主体结构和关键函数名称必须 100% 相似，关键函数相似度必须达到 80% 以上（只变化空格的行不计入差异）。开始迁移、抽取或提交前必须对照 `master` 原文件检查；不满足该要求的迁移切片全部返工重做，不能继续在错误抽象上叠加修改。
