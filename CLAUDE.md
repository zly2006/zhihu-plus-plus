...
- 强制性 subagent 生命周期规则：只要启动了 subagent，主 agent 就必须等待所有仍存活的 subagent 工作完成；接手已有上下文、摘要或环境中列出的 subagent 时，也必须先确认其状态，仍存活的同样要等待完成。只要任何 subagent 还活着，主 agent 就不能自行决策、不能提交、不能宣布完成、不能发送最终回复，也不能继续推进会受该 subagent 结论影响的迁移；等待完成是唯一默认路径，不能因为本地已有判断、等待较久、想继续推进、或认为结果大概率不重要而绕过。不得让 subagent 成为孤儿 agent。任一 subagent 仍存活时，主 agent 不能把等待中的审查结果当作可忽略项。若 subagent 超时但仍存活，应继续 `wait_agent`、补充输入或调整任务，不能直接转为本地主观判断。主动关闭不是常规退路；只有用户明确取消、任务已作废、或该 subagent 的结论经书面说明已不再可能影响当前工作时，才能主动关闭，并必须在回复或相关审查文档中记录关闭原因。每次准备提交、宣布完成、停止当前回合或发送最终回复前，都必须盘点本回合和接手上下文中的 subagent：仍存活的必须 `wait_agent` 到完成，不能静默遗留。
...
- 对两个以上互不重叠的迁移 lane，默认优先并行推进：能用 subagent 时按文件所有权拆分给 subagent；不能用 subagent 时也要按 lane 批量审查、批量验证，避免串行地反复读同一批文件。并行 subagent 不是可遗留的后台任务；一旦启动或接手，就必须按上面的生命周期规则等待完成并消费结论，不能留下孤儿 agent。
...
- `ZhihuMain` 必须参考 `master` 保持大函数结构：shared `ZhihuMain` 自己负责 `NavHost` 和全部 `composable<...>` route 注册。不要把 route graph 拆成 `androidZhihuMainRouteContent`、screen registry 或大注入表；Android/desktop 只注入具体页面实现、Activity/ViewModel 创建、偏好读取、WebView/Toast/Dialog 等平台副作用。
