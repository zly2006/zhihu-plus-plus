...
### Compose Desktop 资源运行链路

当 macOS 直接运行的可执行文件报 Compose 资源缺失，而资源文件已存在于共享资源目录时，必须先核对资源生成、依赖资源聚合、运行目录和 Gradle `run` 任务的 staging 链路。不得把已有资源替换为无关图标或删除调用来掩盖打包问题；只有确认资源不应属于该目标时，才能调整 UI 依赖。例子：共享页面读取应用图标失败时，应检查可执行文件旁是否存在期望的资源目录以及运行任务是否复制了依赖模块资源，而不是把图标直接改成通用符号。
...
允许临时用 instrument test 截图取证，但取图后必须恢复测试源码原状；不得把仅用于留图的测试、文件输出或“截图非空”断言留在提交中。需要长期保留的像素回归断言可以在内存中取图并检查像素。完整测试治理流程见 `.agents/skills/zhihu-instrument-test-governance/SKILL.md`。
