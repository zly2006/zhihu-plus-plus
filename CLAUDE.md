...
- 修改手势灵敏度时，必须先区分方向边界和参数叠加效应：如果需求只要求横向更容易触发，就不能顺手改纵向；如果已经降低触发阈值，就不要再同时降低阻尼，否则两处参数会叠加放大手感变化。例子：一个横向翻页组件和一个纵向过滑组件共用相似的“阈值 + 阻尼”结构，正确做法是只在横向组件里做不超过需求比例的小幅阈值调整，并保留阻尼和纵向组件原行为。
