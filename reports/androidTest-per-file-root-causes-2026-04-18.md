...
| `DailyScreenInstrumentedTest.kt` | Suite-only fail | Injected Daily screen state is not surfacing the expected tagged nodes during connected run. This points to incomplete isolation from production `DailyScreen` setup or semantics mismatch. |
