...
fun PaginationViewModel<*>.refresh(context: Context) {
    refresh(SharedAndroidPaginationEnvironment(context, allowGuestAccess))
}

fun PaginationViewModel<*>.loadMore(context: Context) {
    loadMore(SharedAndroidPaginationEnvironment(context, allowGuestAccess))
}

fun PaginationViewModel<*>.httpClient(context: Context): HttpClient =
    httpClient(SharedAndroidPaginationEnvironment(context, allowGuestAccess))
