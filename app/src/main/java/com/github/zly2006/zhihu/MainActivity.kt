...
//                        val message = intent.data!!.getQueryParameter("message")
        ...
        textToSpeech = TextToSpeech(this) { status ->
            if (status == TextToSpeech.SUCCESS) {
                ...
                if (availableEngines.contains(picoEngine) || availableEngines.contains(sherpaEngine)) {
                    // 如果Pico TTS可用，切换到Pico引擎, Sherpa TTS可用，切换到Sherpa引擎
                    initializeTtsSettings()
                    ttsEngine = if(availableEngines.contains(picoEngine)) TtsEngine.Pico else TtsEngine.Sherpa
