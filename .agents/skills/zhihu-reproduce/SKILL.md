...
- 分页交互：必须同时取证下一页由触底、显式按钮还是其他事件触发；项目已有连续列表默认沿用触底自动分页，不能为了调试方便把“加载更多”按钮交付成产品交互
...
当任务要复刻知乎官方 Android 客户端的 UI 或交互，或者用户明确要求核对官方客户端时，必须在 AVD 中打开真正的官方应用完成验证，不能用 Web 页面、API 返回或 Zhihu++ 自身界面代替。验证前先记录 AVD 名称和 Android API 级别，再通过包管理器确认包名为 `com.zhihu.android` 并记录 `versionName`、`versionCode`；同时排除 `com.github.zly2006.zhplus` 及其变体，避免把被测应用误认成官方客户端。
...
修复登录、凭证刷新或会话初始化问题时，必须先把用户可达的成功态写成验收契约，再从成功态反推请求链路。检测到关键凭证缺失并报错只能防止错误状态落库，不能算修复；如果用户原本无法登录，这种改动仍然让用户无法登录。必须继续查明官方或现有可用链路如何取得完整凭证，主动完成获取、验证和持久化，并用真实账号证明最终会话能进入应用且后续认证请求可用。例子：移动端登录返回账号 token，但网页签名还需要设备 Cookie 时，正确修复是找到并复刻能下发该 Cookie 的会话交换请求，再验证账号信息与签名接口；只在 Cookie 为空时抛出异常，是失败保护，不是登录成功方案。
