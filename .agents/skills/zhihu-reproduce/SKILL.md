...
同步能力必须先明确它服务的用户行为，再决定完整性与请求预算。为避免重复推荐同步已读记录时，不能把首页串行等待全量历史、后台同步后刷新整页，或无协议证据的周期性全量扫描作为默认方案。应验证远端排序与增量边界，把成功进度和尝试时间分开保存，并证明失败不会推进进度、慢请求不会无限阻塞首页、同步结果不会打断当前阅读位置。
